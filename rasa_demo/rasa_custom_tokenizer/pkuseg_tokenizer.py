...
class pkuseg(Tokenizer):
    ...
    def _load_custom_dictionary(path: Text) -> None:
        ...
        """
        import pkuseg
        pkuseg_userdicts = glob.glob(f"{path}/*")
        for pkuseg_userdict in pkuseg_userdicts:
            logger.info(f"Loading pkuseg User Dictionary at {pkuseg_userdict}")
            pkuseg.pkuseg(pkuseg_userdict) # ?
            # jieba.load_userdict(pkuseg)
    ...
    def tokenize(self, message: Message, attribute: Text) -> List[Token]:
        ...
        #tokenized = pkuseg.pkuseg.cut(text)
        seg = pkuseg.pkuseg()
        tokenized = seg.cut(text)
